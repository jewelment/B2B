...
export default function GridClient({ catalog }: { catalog: any }) {
  ...
  return (
    <div className={`min-h-screen ${themeClasses.bg} selection:bg-[var(--brand-primary)] selection:text-white pb-32 transition-colors duration-700`}>
      ...
      <div className="relative w-full h-[60vh] md:h-[70vh] flex flex-col items-center justify-center overflow-hidden border-b border-white/10">
        <div className="absolute inset-0 z-0">
          <img src={config.frontCover || "https://images.unsplash.com/photo-1599643478514-4a4e09b52342?auto=format&fit=crop&w=2000&q=80"} alt="Cover Background" className="w-full h-full object-cover blur-sm scale-105 opacity-20" />
          <div className={`absolute inset-0 ${isDark ? 'bg-gradient-to-b from-[#0a0a0a]/50 via-[#0a0a0a]/80 to-[#0a0a0a]' : 'bg-gradient-to-b from-white/50 via-white/80 to-[#f8f9fa]'}`}></div>
        </div>
        ...
        <div className="relative z-10 text-center px-6 max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-8 duration-1000">
          ...
          <p className={`text-sm md:text-base ${themeClasses.textMuted} tracking-wide font-medium`}>
            Curated exclusively for <span className={themeClasses.textMain}>{catalog.clientId || 'Valued Partner'}</span>
          </p>
          
          <div className="mt-12 flex items-center justify-center gap-4">
            <button className={`w-12 h-12 rounded-full flex items-center justify-center backdrop-blur-md border transition-all ${isDark ? 'bg-white/5 border-white/10 text-white hover:bg-white/10' : 'bg-black/5 border-black/10 text-black hover:bg-black/10'}`} title="Share Internally">
              <IconShare />
            </button>
            <button 
              onClick={() => window.open(`/api/catalog/${catalog.id}/pdf`, '_blank')}
              className={`px-6 py-3 text-[10px] font-bold uppercase tracking-widest rounded-full flex items-center gap-2 backdrop-blur-md border transition-all ${isDark ? 'bg-white/5 border-white/10 text-white hover:bg-white/10' : 'bg-black/5 border-black/10 text-black hover:bg-black/10'}`}
              title="Download PDF"
            >
          ...
          {catalog.items.map((item: any, idx: number) => {
            ...
            return (
              ...
              >
                ...
                {/* Product Image */}
                <div className="relative aspect-[4/5] bg-white overflow-hidden cursor-pointer" onClick={() => toggleSelection(product.designCode)}>
                  <img src={product.mainImage || product.description} alt={product.title} className="w-full h-full object-cover mix-blend-multiply group-hover:scale-110 transition-transform duration-700 ease-in-out" />
                ...
                {/* Product Meta */}
                <div className={`p-6 flex flex-col flex-1 ${isDark ? 'bg-gradient-to-t from-[#0a0a0a] to-transparent absolute bottom-0 w-full' : 'bg-white'}`}>
                  <div className="flex justify-between items-start mb-2">
                    <p className={`text-[10px] font-mono tracking-widest ${isDark ? 'text-gray-300' : 'text-gray-500'}`}>{product.designCode}</p>
                    <span className="text-[9px] font-bold uppercase tracking-wider bg-[#4e080f]/10 text-[#4e080f] px-2 py-0.5 rounded">{product.metalPurity || '18KT'}</span>
                  </div>
                  ...
                  <div className="mt-auto flex justify-between items-end">
                    <div>
                      ...
                      <p className={`text-lg font-light ${isDark ? 'text-white' : 'text-gray-900'}`}>{formatPrice(product.estimatedPrice)}</p>
                    </div>
                  </div>
                </div>
              </div>
