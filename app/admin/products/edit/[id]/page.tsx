...
export default function SingleProductEditAccurateUI() {
  ...
  const [variantsPagination, setVariantsPagination] = useState({ total: 0, page: 1, limit: 50, totalPages: 1 });
  const [activeVariantMedia, setActiveVariantMedia] = useState<any>(null);
  ...
  const fetchOptionSets = async () => {
    try {
      const res = await fetch('/api/admin/parameters/option-sets');
      if (res.ok) {
        const data = await res.json();
        setAvailableOptionSets(data.data || []);
      }
    } catch (e) {
      console.error(e);
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.data && data.data.length > 0) {
          // Format for UI
          const pos = data.data[0].optionSet;
          setOptionSet({
            name: pos.title,
            options: pos.items.map((item: any) => ({
              name: item.option.frontendName,
              values: item.option.values.map((v: any) => v.name)
            }))
          });
        }
      }
    ...
    try {
      ...
      if (res.ok) {
        ...
        const mappedVariants = (data.data || []).map((v: any) => ({
          id: v.id,
          name: v.optionValues.map((ov: any) => ov.optionValue.name).join(' • '),
          sku: v.sku,
          netWeight: v.weightGrams?.toFixed(3) || '3.500',
          grossWeight: (v.weightGrams ? v.weightGrams + 0.5 : 4.0).toFixed(3),
          price: v.price || Math.floor(15000 + Math.random() * 50000),
          discount: '10%'
        }));
        setVariants(mappedVariants);
        setVariantsPagination(data.pagination || { total: 0, page: 1, limit: 50, totalPages: 1 });
      }
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleImportOptionSet = async (setId: string) => {
    setIsImporting(true);
    try {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-base)] text-[var(--text-main)] font-sans transition-colors duration-400 pb-20">
            ...
            <div className="space-y-8 animate-in fade-in duration-300">
              <div className="surface-panel p-6">
                ...
                {!isSingleVariant && optionSet.options ? (
                  ...
                  <div className="flex flex-col items-center justify-center p-10 bg-[var(--bg-base)] border border-[var(--border-color)] border-dashed rounded-xl mb-6">
                    <svg className="w-10 h-10 text-[var(--text-muted)] opacity-50 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 002-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg>
                    <p className="text-sm font-bold text-[var(--text-main)] mb-1">Select an Option Set</p>
                    <p className="text-xs text-[var(--text-muted)] mb-6 text-center max-w-sm">Choose an existing Option Set to automatically populate variant parameters for this product.</p>
                    <div className="w-full max-w-md mb-6">
                      <CustomSelect 
                        label="Select Option Set" 
                        value={optionSet.name || ''} 
                        onChange={(val: string) => {
                          const found = availableOptionSets.find(s => s.title === val);
                          if (found) {
                            handleImportOptionSet(found.id);
                          }
                        }}
                        options={availableOptionSets.map(s => s.title)} 
                        className="bg-[var(--bg-surface)] text-left w-full"
                      />
                    </div>
                    <div className="flex gap-4">
                      <button onClick={() => console.log('Create Option Set pending')} className="px-5 py-2.5 bg-[var(--bg-surface)] text-[var(--text-main)] border border-[var(--border-color)] hover:border-[var(--brand-primary)] hover:text-[var(--brand-primary)] font-bold text-[10px] uppercase tracking-widest rounded-full transition-all shadow-sm">Create Option Set</button>
                      <button onClick={() => setIsImportOptionsModalOpen(true)} className="px-5 py-2.5 bg-[var(--brand-primary)] text-[var(--brand-text)] font-bold text-[10px] uppercase tracking-widest rounded-full hover:opacity-90 transition-opacity shadow-sm shimmer-hover">Import Option Set</button>
                    </div>
                  </div>
                )}
              </div>
                        ...
                        {variants.map((variant) => (
                          <tr key={variant.id} className="hover:bg-black/5 dark:hover:bg-white/5 transition-colors group">
                            ...
                            <td className="py-3 px-4 text-center border-r border-[var(--border-color)]/50 align-middle">
                              <div onClick={() => setActiveVariantMedia(variant)} className="w-8 h-8 rounded-lg bg-[var(--bg-base)] border border-[var(--border-color)] mx-auto flex items-center justify-center text-[var(--text-muted)] cursor-pointer hover:border-[var(--brand-primary)] hover:text-[var(--brand-primary)] transition-all shadow-sm">
              ...
              <div className="p-6 bg-[var(--bg-surface)]">
                ...
                <div className="space-y-3 max-h-[60vh] overflow-y-auto">
                  {availableOptionSets.map(os => (
                    <div 
                      key={os.id}
                      onClick={() => !isImporting && handleImportOptionSet(os.id)} 
                      className={`p-4 border border-[var(--border-color)] rounded-xl transition-colors ${isImporting ? 'opacity-50 cursor-not-allowed' : 'hover:border-[var(--brand-primary)] hover:bg-[var(--brand-primary)]/5 cursor-pointer'}`}
                    >
                      <h4 className="font-bold text-sm text-[var(--text-main)]">{os.title}</h4>
                      <p className="text-xs text-[var(--text-muted)] mt-1">
                        {isImporting ? 'Generating matrix...' : os.items.map((item: any) => `${item.option.frontendName} (${item.option.values.length})`).join(' • ')}
                      </p>
                    </div>
                  ))}
                  {availableOptionSets.length === 0 && (
                    <p className="text-sm text-[var(--text-muted)] p-4 text-center">No Option Sets found. Please create one in parameters.</p>
                  )}
                </div>
              </div>
            </motion.div>
      ...
      {/* PHASE 18.3 VARIANT MEDIA UPLOAD MODAL */}
      <AnimatePresence>
        {activeVariantMedia && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center">
            <motion.div
              ...
              onClick={() => setActiveVariantMedia(null)}
            />
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              ...
              className="relative w-full max-w-2xl bg-[rgba(255,255,255,0.9)] dark:bg-[rgba(15,15,15,0.9)] backdrop-blur-xl rounded-2xl shadow-[0_4px_30px_rgba(0,0,0,0.1)] border border-white/20 dark:border-white/10 overflow-hidden flex flex-col mx-4"
            >
              {/* Modal Header */}
              <div className="px-6 py-5 border-b border-[var(--border-color)] flex items-center justify-between bg-gradient-to-r from-transparent to-black/5 dark:to-white/5">
                <div>
                  ...
                  <p className="text-xs text-[var(--text-muted)] mt-1 font-mono">{activeVariantMedia.name}</p>
                </div>
                <button
                  onClick={() => setActiveVariantMedia(null)}
                  className="p-2 bg-black/5 hover:bg-black/10 dark:bg-white/5 dark:hover:bg-white/10 rounded-full transition-all text-[var(--text-muted)] hover:text-red-500 hover:scale-105 active:scale-95"
                >
                  <IconClose />
                </button>
              </div>
              ...
              <div className="p-6 overflow-y-auto max-h-[60vh] custom-scrollbar">
                ...
                {/* Upload Zone */}
                <div className="w-full h-32 border-2 border-dashed border-[var(--border-color)] rounded-xl flex flex-col items-center justify-center text-[var(--text-muted)] hover:border-[var(--brand-primary)] hover:bg-[var(--brand-primary)]/5 transition-all cursor-pointer group mb-6 relative overflow-hidden">
                  <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-10 mix-blend-overlay pointer-events-none"></div>
                  <svg className="w-8 h-8 mb-2 group-hover:scale-110 transition-transform group-hover:text-[var(--brand-primary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>
                  <span className="text-sm font-semibold group-hover:text-[var(--brand-primary)] transition-colors uppercase tracking-widest text-[10px]">Drop High-Res Variant Assets Here</span>
                </div>

                {/* Simulated Media Grid */}
                <div>
                  <h4 className="text-xs font-bold uppercase tracking-widest text-[var(--text-muted)] mb-3">Linked Assets</h4>
                  <div className="grid grid-cols-3 sm:grid-cols-4 gap-4">
                    <div className="aspect-square bg-[var(--bg-base)] border border-[var(--border-color)] rounded-xl flex flex-col items-center justify-center text-[var(--text-muted)] relative group cursor-pointer overflow-hidden shadow-sm hover:shadow-md transition-all">
                       <svg className="w-8 h-8 text-amber-500/50 group-hover:scale-110 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><circle cx="12" cy="12" r="8" strokeWidth="1"/></svg>
                       <span className="absolute bottom-2 right-2 bg-amber-500 text-white text-[8px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wider">Primary</span>
                       <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity bg-black/50 text-white p-1 rounded-full hover:bg-red-500"><IconClose /></div>
                    </div>
                    <div className="aspect-square bg-[var(--bg-base)] border border-[var(--border-color)] rounded-xl flex flex-col items-center justify-center text-[var(--text-muted)] relative group cursor-pointer overflow-hidden shadow-sm hover:shadow-md transition-all">
                       <svg className="w-8 h-8 text-amber-500/50 group-hover:scale-110 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><rect x="4" y="4" width="16" height="16" rx="2" strokeWidth="1"/></svg>
                       <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity bg-black/50 text-white p-1 rounded-full hover:bg-red-500"><IconClose /></div>
                    </div>
                    {/* Add new button */}
                    <div className="aspect-square bg-transparent border border-dashed border-[var(--border-color)] rounded-xl flex items-center justify-center text-[var(--text-muted)] cursor-pointer hover:border-[var(--brand-primary)] hover:bg-[var(--brand-primary)]/5 transition-colors">
              ...
              {/* Modal Footer */}
              <div className="px-6 py-4 border-t border-[var(--border-color)] bg-black/5 dark:bg-white/5 flex justify-end gap-3">
                <button
                  onClick={() => setActiveVariantMedia(null)}
                  className="px-5 py-2 text-xs font-bold uppercase tracking-widest text-[var(--text-muted)] hover:text-[var(--text-main)] transition-colors hover:bg-black/5 dark:hover:bg-white/5 rounded-full"
                >
                  CANCEL
                </button>
                <button
                  onClick={() => setActiveVariantMedia(null)}
                  className="px-6 py-2 text-xs font-bold uppercase tracking-widest bg-[var(--brand-primary)] text-white rounded-full shadow-[0_4px_15px_rgba(var(--brand-primary-rgb),0.3)] hover:scale-[1.02] active:scale-[0.98] transition-all shimmer-hover"
                >
                  SAVE MEDIA
                </button>
              </div>
