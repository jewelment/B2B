...
export default function AdminCatalogsDashboard() {
  ...
  const [config, setConfig] = useState({
    ...
    frontCover: '',
    backCover: '',
    backCoverText: 'Ashok Jewels Fine Wholesale',
    viewerBackground: 'UNSPLASH_WHITE_LINES',
  ...
  const fetchData = async () => {
    try {
      ...
      const invRes = await fetch('/api/inventory?limit=5000'); 
      if (invRes.ok) {
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 items-start">
        <div className="xl:col-span-1 bg-[var(--glass-bg)] backdrop-blur-xl border border-[var(--glass-border)] rounded-2xl p-6 shadow-sm">
          ...
          <form onSubmit={handleCreate} className="space-y-5">
            ...
            <div>
              <label className="block text-[10px] font-bold text-[var(--text-muted)] uppercase mb-2">Subtitle (Optional)</label>
            ...
            <div>
              <div className="flex justify-between items-end mb-2">
                ...
                <button type="button" onClick={() => {
                  setEditingCatalogId(null);
                  // Intentionally keeping campaignName and clientSearch so data carries over
                  setShowBuilder(true);
                    ...
                    <td className="px-6 py-5 text-right">
                      <div className="flex justify-end items-center gap-2">
                        <button title="Edit Configuration" onClick={() => { 
                          ...
                          setShowBuilder(true); 
                        }} className="w-8 h-8 flex items-center justify-center rounded-md border border-[var(--border-color)] bg-transparent text-[var(--text-muted)] hover:text-[var(--text-main)] hover:bg-[var(--bg-surface)] transition-all"><IconEdit /></button>
                        
                        <Link href={`/catalog/view/${cat.id}`} target="_blank" title="Standard Grid View" className="w-8 h-8 flex items-center justify-center rounded-md border border-[var(--border-color)] bg-transparent text-[var(--text-muted)] hover:text-[var(--text-main)] hover:bg-[var(--bg-surface)] transition-all"><IconEye /></Link>
                        
                        <Link href={`/catalog/flipbook/${cat.id}`} target="_blank" title="Immersive 3D Flipbook" className="w-8 h-8 flex items-center justify-center rounded-md border border-[var(--border-color)] bg-transparent text-[var(--text-muted)] hover:text-[var(--text-main)] hover:bg-[var(--bg-surface)] transition-all"><IconBook /></Link>
                        
                        <Link href={`/admin/analytics?catalogId=${cat.id}`} title="View Analytics" className="w-8 h-8 flex items-center justify-center rounded-md border border-[var(--border-color)] bg-transparent text-[var(--text-muted)] hover:text-[var(--brand-primary)] hover:border-[var(--brand-primary)] hover:bg-[var(--brand-primary)]/5 transition-all"><IconChart /></Link>
                        
                        <a href={`/api/catalog/${cat.id}/pdf`} download title="Download PDF" className="w-8 h-8 flex items-center justify-center rounded-md border border-[var(--border-color)] bg-transparent text-[var(--text-muted)] hover:text-[var(--text-main)] hover:bg-[var(--bg-surface)] transition-all"><IconDownload /></a>
                        
                        <button onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/catalog/view/${cat.id}`); alert('Link Copied!'); }} title="Copy Link" className="w-8 h-8 flex items-center justify-center rounded-md border border-[var(--border-color)] bg-transparent text-[var(--text-muted)] hover:text-[var(--brand-primary)] hover:border-[var(--brand-primary)] hover:bg-[var(--brand-primary)]/5 transition-all"><IconShare /></button>
                        
                        <button onClick={() => handleDeleteCatalog(cat.id)} title="Delete Catalog" className="w-8 h-8 flex items-center justify-center rounded-md border border-[var(--border-color)] bg-transparent text-[var(--text-muted)] hover:text-red-500 hover:border-red-500 hover:bg-red-50 transition-all"><IconTrash /></button>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <div className="fixed top-0 right-0 bottom-0 left-64 z-50 flex items-center justify-center p-4 sm:p-8 animate-in fade-in duration-200">
          ...
          <div className="relative w-full max-w-[95vw] lg:max-w-7xl h-[90vh] bg-[var(--bg-base)] border border-[var(--glass-border)] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <div className="flex border-b border-[var(--border-color)] bg-[var(--bg-base)] px-8 z-10">
              ...
              <button onClick={() => setBuilderTab('APPEARANCE')} className={`px-6 py-4 text-xs font-bold uppercase tracking-widest border-b-2 transition-all ${builderTab === 'APPEARANCE' ? 'border-[var(--brand-primary)] text-[var(--brand-primary)]' : 'border-transparent text-[var(--text-muted)] hover:text-[var(--text-main)]'}`}>2. Settings & Media</button>
                  ...
                  <div className={`grid ${getGridClass()} gap-4 md:gap-6 content-start`}>
                    {paginatedInventory.length === 0 ? (
                      ...
                      paginatedInventory.map((item, i) => {
                        ...
                        return (
                          <div key={i} onClick={() => toggleVisualSelection(item.designCode)} className={`relative group rounded-xl border-2 cursor-pointer transition-all overflow-hidden ${isSelected ? 'border-blue-500 ring-2 ring-blue-500/30 shadow-lg scale-[1.02]' : 'border-[var(--border-color)] hover:border-blue-500/50'}`}>
                            {isSelected && (
                              <div className="absolute top-2 right-2 w-6 h-6 bg-blue-500 text-white rounded-full flex items-center justify-center z-10 shadow-lg animate-in zoom-in-90">
                                <IconCheck />
                              </div>
                            )}
                ...
                <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="bg-[var(--bg-surface)] p-6 rounded-2xl border border-[var(--border-color)]">
                      <h4 className="text-xs font-bold uppercase tracking-widest text-[var(--text-main)] border-b border-[var(--border-color)] pb-3 mb-5">Catalog Details</h4>
                      <div className="space-y-5">
                        <div>
                          <label className="block text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider mb-2">Catalog Title</label>
                          <input 
                            type="text" 
                            value={campaignName} 
                            onChange={(e) => setCampaignName(e.target.value)} 
                            placeholder="e.g. Summer Collection 2026"
                            className="w-full px-4 py-2.5 bg-[var(--bg-base)] border border-[var(--border-color)] rounded-xl text-sm font-semibold text-[var(--text-main)] focus:ring-1 focus:ring-[var(--brand-primary)] outline-none" 
                          />
                        </div>
                        <div>
                          <label className="block text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider mb-2">Target Client / Subtitle (Optional)</label>
                          <input 
                            type="text" 
                            value={clientSearch} 
                            onChange={(e) => setClientSearch(e.target.value)} 
                            placeholder="e.g. Malabar Gold"
                            className="w-full px-4 py-2.5 bg-[var(--bg-base)] border border-[var(--border-color)] rounded-xl text-sm font-medium text-[var(--text-main)] focus:ring-1 focus:ring-[var(--brand-primary)] outline-none" 
                          />
                        </div>
                        <div>
                          <label className="block text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider mb-2">Back Cover Content</label>
                          <input 
                            type="text" 
                            value={config.backCoverText || ''} 
                            onChange={(e) => setConfig({...config, backCoverText: e.target.value})} 
                      ...
                      <div className="space-y-5">
                        ...
                        <div>
                          ...
                          <select value={config.desktopItemsPerPage} onChange={(e) => setConfig({...config, desktopItemsPerPage: Number(e.target.value)})} className="w-full bg-[var(--bg-base)] border border-[var(--border-color)] text-xs py-3 px-4 rounded-xl focus:ring-1 focus:ring-[var(--brand-primary)] outline-none cursor-pointer text-[var(--text-main)]">
                            ...
                            <option value={0}>Smart Auto-Align</option>
                          </select>
                        </div>
